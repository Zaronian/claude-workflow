...
5. **Fix real findings**, re-run the full suite, then re-review **the delta only** (the fix commits, plus the original findings list) with one Opus reviewer. Keep iterating — fix, re-run, re-review the delta — while the verdict is CHANGES. **A fourth post-verification CHANGES verdict (three fix rounds have not converged) → stop and ask the user** with the findings; do not merge.
