#!/bin/bash
# handoff.sh — store + copy anything a Claude session hands to the user.
#
# Usage:  <body on stdin> | handoff.sh <kind> <business/project> [--session ID] [--no-copy]
#   kind: handoff | need-from-you | command | note
#
# Writes ~/.claude/handoffs/sessions/<session_id>.md (header + body),
# copies it to ~/.claude/handoffs/latest.md, rewrites index.md, and pbcopys
# the body. Sessions identify themselves via $CLAUDE_CODE_SESSION_ID (set in
# Claude Code's Bash tool); --session overrides. Retrieval: /handoff, or
#   alias hand='tail -n +7 ~/.claude/handoffs/latest.md | pbcopy'
set -euo pipefail

KIND="${1:-}"; PROJECT="${2:-}"
[ -z "$KIND" ] || [ -z "$PROJECT" ] && { echo "usage: handoff.sh <kind> <business/project> [--session ID] [--no-copy]" >&2; exit 2; }
shift 2
SESSION="${CLAUDE_CODE_SESSION_ID:-}"; COPY=1
while [ $# -gt 0 ]; do
  case "$1" in
    --session) SESSION="$2"; shift 2 ;;
    --no-copy) COPY=0; shift ;;
    *) echo "unknown option: $1" >&2; exit 2 ;;
  esac
done
[ -z "$SESSION" ] && SESSION="unknown-$(date +%Y%m%d-%H%M%S)"

case "$KIND" in handoff|need-from-you|command|note) ;; *) echo "kind must be handoff|need-from-you|command|note" >&2; exit 2 ;; esac

ROOT="$HOME/.claude/handoffs"; mkdir -p "$ROOT/sessions"
BODY="$(cat)"
[ -z "$BODY" ] && { echo "empty body on stdin" >&2; exit 2; }
NOW="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
FILE="$ROOT/sessions/$SESSION.md"

{
  printf 'session: %s\nproject: %s\ncwd: %s\nwritten: %s\nkind: %s\n\n' "$SESSION" "$PROJECT" "$PWD" "$NOW" "$KIND"
  printf '%s\n' "$BODY"
} > "$FILE"
cp "$FILE" "$ROOT/latest.md"

# index.md — one line per session, newest first
{
  echo "# Handoff registry — newest first (written by _system/scripts/handoff.sh)"
  echo
  for f in $(ls -t "$ROOT/sessions"/*.md 2>/dev/null); do
    s=$(sed -n 's/^session: //p' "$f" | head -1)
    p=$(sed -n 's/^project: //p' "$f" | head -1)
    k=$(sed -n 's/^kind: //p' "$f" | head -1)
    w=$(sed -n 's/^written: //p' "$f" | head -1)
    echo "- $w · $p · $k · $s"
  done
} > "$ROOT/index.md"

if [ "$COPY" -eq 1 ] && command -v pbcopy >/dev/null 2>&1; then
  printf '%s' "$BODY" | pbcopy
fi
echo "stored: $FILE ($KIND, $PROJECT)"
