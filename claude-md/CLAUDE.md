...
- **Worktree ownership.** Whichever harness created a git worktree retires it. Claude removes only worktrees a Claude session created *and recorded* (roadmap Session Context / work log / restart note name the worktree and its branch) — no record, no removal. Record every worktree you create at the checkpoint that creates it. Another harness's entry file inside a folder is not proof that harness opened it (some write one into every checkout they find), so when ownership is unclear, leave the folder and tell the user. Never work inside a worktree the other harness created; removing a worktree deletes its uncommitted edits, which is the whole risk.
- **Two harnesses, four layers.** When two coding agents share the same repos and knowledge base, coordinate by layer, with no shared log. (1) *Harness config* — each agent's entry files are its own; facts are shared through `company.md`. Shared docs (the reference guide, templates, knowledge bases) stay harness-neutral: model names and harness-specific mechanics (agent-call syntax, model flags, hook internals) belong in each agent's own entry files; procedure names such as `/pr-review` are shared vocabulary and may appear in shared docs. (2) *Uncommitted work* — lives only in worktrees; neither agent edits inside a folder the other opened. Ownership is the folder name (Claude: `<repo>-wt-<topic>`; the other agent uses its own suffix); `git worktree list` is the registry. The main checkout is nobody's scratch space — branch into a worktree for real work and leave it clean. (3) *Committed work* — shared property; branches and PRs are the log, and either agent may change files the other committed on its own branch. (4) *Projects* — one project, one agent at a time; switch only at a checkpoint. The roadmap Session Context carries a `**Tool:**` line naming the current owner; read it before picking up a `>>>` and set it when you take one.
...
- **Default is single-session sequential work.** Use *background* subagents (long-running, session keeps working) only for an independent build step with a written brief and a test-based exit condition — never for research/exploration, never a workflow fan-out. *Foreground* agents the session waits on — the `/pr-review` gate's reviewer, finders, and verifiers; a bounded Explore or research lookup — are not background agents: they are allowed, and they do not count against the cap below.
- **Cap: two background agents at a time** (foreground gate and lookup agents excluded) — standing allowance, no per-launch approval. Before launching, state the rough cost in session-equivalents and check the remaining session budget (Pacing & budget). The user can stop any agent at any time.
- **Bounded brief** (required): goal; definition of done (tests/commands); files/dirs in scope; constraints (ADRs, patterns); model (Opus unless the brief states why not); its own worktree; report format (≤1 page: outcome, evidence, PR link, open questions); **stop conditions** — stop and report if tests fail after two attempts, if a decision is needed, or if the approach changes.
