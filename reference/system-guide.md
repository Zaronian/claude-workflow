...
7. **Handoff** — Only when the session ends, the business switches, or the user asks: next deliverable, 2–3 sentences of bigger picture, docs to read, the `>>>` task, through `handoff.sh handoff <business/project>` (pbcopy + `~/.claude/handoffs/`). `/handoff` re-surfaces it later for one file read.
